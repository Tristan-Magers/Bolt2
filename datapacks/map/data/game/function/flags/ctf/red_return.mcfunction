...
execute if score .testing_mode .data matches 0 if score .mode .data matches 1 if score .tmi .data matches 0 as @a[team=red,distance=..10,dx=1,dy=1.5,dz=1,tag=hasflag] as @s[nbt={Inventory:[{id:"minecraft:iron_ingot",count:2}]},tag=locked_77,tag=playing] run function game:player/unlock/77
