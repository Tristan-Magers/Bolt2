...
# security that values are within expected ranges
scoreboard players set @a[scores={rank_delta_up=..0}] rank_delta_up 0
scoreboard players set @a[scores={rank_delta_down=..0}] rank_delta_down 0

scoreboard players set @a[scores={rank_delta_up=100..}] rank_delta_up 100
scoreboard players set @a[scores={rank_delta_down=100..}] rank_delta_down 100
...
# remove player dif from point dif (without making it go negative)
execute if score .point_dif .metric > .0 .num if score .team_dif .data > .0 .num if score .team_dif .data > .team_dif .data run scoreboard players operation .point_dif_adjusted .metric -= .team_dif .metric
execute if score .point_dif .metric < .0 .num if score .team_dif .data < .0 .num if score .team_dif .data < .team_dif .data run scoreboard players operation .point_dif_adjusted .metric -= .team_dif .metric
...
execute as @a[team=red] if score .point_red .metric = .3 .num run scoreboard players operation @s captures_ranked *= .10 .num
execute as @a[team=blue] if score .point_blue .metric = .3 .num run scoreboard players operation @s captures_ranked *= .10 .num
execute as @a[team=red] unless score .point_red .metric = .3 .num run scoreboard players operation @s captures_ranked *= .15 .num
execute as @a[team=blue] unless score .point_blue .metric = .3 .num run scoreboard players operation @s captures_ranked *= .15 .num
